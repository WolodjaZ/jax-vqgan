...
from modules import vqgan, config
...
def test_quick_gelu(JAX_PRNG):
    ...
    # compare times
    start = time.time(); nn.gelu(x)
    end_rgular = time.time() - start
    
    start = time.time(); vqgan.quick_gelu(x)
...
def test_VectorQuantizer(JAX_PRNG, n_embed, embed_dim):
    ...
    assert z_q is not None; assert min_encoding_indices is not None
    assert z_q.dtype == dtype; assert z_q.shape == x.shape[:-1] + (embed_dim,)
    assert min_encoding_indices.shape == (1, hw_ch*hw_ch);
    assert jnp.min(min_encoding_indices) >= 0; assert jnp.max(min_encoding_indices) <= n_embed-1
    ...
    assert z_q_entry.shape == x.shape; jnp.allclose(z_q_entry, z_q, atol=0.0001)
...
def test_GumbelQuantize(JAX_PRNG, n_embed, embed_dim):
    ...
    z_q, min_encoding_indices, logits = vq.apply({'params': params}, x, rngs={'gumbel': gumble_apply_rng})
    assert z_q is not None; assert min_encoding_indices is not None; assert logits is not None
    assert z_q.dtype == dtype; assert z_q.shape == x.shape[:-1] + (embed_dim,)
    assert min_encoding_indices.shape == (1, hw_ch*hw_ch);
    assert jnp.min(min_encoding_indices) >= 0; assert jnp.max(min_encoding_indices) <= n_embed-1
    assert logits.shape == (1, hw_ch, hw_ch, n_embed); assert logits.dtype == dtype
...
def test_VQModule(JAX_PRNG, act_name, hw_ch, batch_size, use_gumbel):
    ...
    # Check initialization
    params = vqmodule.init({'params': init_rng, 'gumbel': gumble_init_rng, 'dropout': dropout_init_rng}, x, True)['params']
    assert params is not None

    # Check forward pass
    rng, gumble_apply_rng, dropout_apply_rng = jax.random.split(rng, num=3)
    x_recon, z_q, min_encoding_indices, logits = vqmodule.apply({'params': params}, x, rngs={'gumbel': gumble_apply_rng, 'dropout': dropout_apply_rng}, deterministic=True)
    assert z_q is not None; assert min_encoding_indices is not None
    if use_gumbel:
        assert logits is not None;
    assert z_q.dtype == dtype; x_recon.dtype == dtype
    assert x_recon.shape == x.shape
    
    # Check decode_code
    def get_codebook_entry(model):
        return model.decode_code(min_encoding_indices, z_shape=z_q.shape)
    
    x_recon_code = nn.apply(get_codebook_entry, model())({'params': params})
    assert x_recon_code.shape == x_recon.shape; jnp.allclose(x_recon_code, x_recon, atol=0.0001)
...
def test_VQModel(JAX_PRNG, hw_ch, batch_size, use_gumbel):
    ...
    # Check instantiation
    vqmodel = vqgan.VQModel(config=config_vqgan, 
    ...
    # Check initialization
    rng, init_rng = jax.random.split(rng)
    params = vqmodel.init_params(init_rng)
    assert params is not None
    ...
    # Check forward pass
    rng, gumble_apply_rng, dropout_apply_rng = jax.random.split(rng, num=3)
    x_recon, z_q, indices, logits = vqmodel(x, 
                                            params=params, 
                                            dropout_rng=dropout_apply_rng,
                                            gumble_rng=gumble_apply_rng,
                                            traun=False)
    assert z_q is not None; assert indices is not None
    if use_gumbel:
        assert logits is not None;
    assert z_q.dtype == dtype; x_recon.dtype == dtype
    assert x_recon.shape == x.shape
    ...
    # check encode
    z_q_encoder, indices_encode, logits_encode = vqmodel.encode(x,
                                          params=params, 
                                          dropout_rng=dropout_apply_rng,
                                          gumble_rng=gumble_apply_rng,
                                          traun=False)
    ...
    # check decode
    x_recon_decode = vqmodel.decode(z_q,
                                    params=params, 
                                    dropout_rng=dropout_apply_rng,
                                    gumble_rng=gumble_apply_rng,
                                    traun=False)
    ...
    # check decode_code
    x_recon_decode_code = vqmodel.decode_code(indices,
                                              params=params)
    assert jnp.allclose(x_recon, x_recon_decode_code, atol=0.0001)
