...
class ResNetBlock(nn.Module):
    ...
    def setup(self):
        ...
        self.out_channels_ : int = self.in_channels if self.out_channels is None else self.out_channels
...
class AttnBlock(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        # get query, key, value
        q = nn.Conv(self.in_channels, 
                    kernel_size=(1, 1),
                    strides=(1, 1),
                    padding="VALID",
                    dtype=self.dtype)(h_)
        k = nn.Conv(self.in_channels, 
                    kernel_size=(1, 1),
                    strides=(1, 1),
                    padding="VALID",
                    dtype=self.dtype)(h_)
        v = nn.Conv(self.in_channels, 
                    kernel_size=(1, 1),
                    strides=(1, 1),
                    padding="VALID",
                    dtype=self.dtype)(h_)
